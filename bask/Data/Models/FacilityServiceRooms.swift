//
//  FacilityServiceRooms.swift
//  bask
//
//  Created by TEHSEEN ABBAS on 03/08/2022.
//
//
//   let facilityServiceRooms = try? newJSONDecoder().decode(FacilityServiceRooms.self, from: jsonData)

import Foundation

// MARK: - FacilityServiceRooms
struct FacilityServiceRooms: Codable {
    let data: FacilityServiceRoomsData
    let error: Bool
    let message: String
}

// MARK: - DataClass
struct FacilityServiceRoomsData: Codable {
...
struct OtherData: Codable {
    let length, width, capacity, noOfBathRooms, noOfShowers: Int?
}
